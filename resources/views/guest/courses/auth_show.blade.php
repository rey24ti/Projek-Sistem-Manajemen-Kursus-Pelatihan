      ...
      <div class="card-body">
        ...
        <div class="row mt-4">
          <div class="col-md-6">
            ...
            <p class="mb-2"><strong>Kategori:</strong> <span class="badge bg-gradient-primary">{{ $course->category->name }}</span></p>
            <p class="mb-2"><strong>Trainer:</strong> {{ $course->trainer->name }}</p>
            <p class="mb-2"><strong>Status:</strong> <span class="badge bg-gradient-{{ $course->status_badge }}">{{ ucfirst($course->status) }}</span></p>
            @if($course->passing_score)
              <p class="mb-2"><strong>Nilai Minimum Kelulusan:</strong> {{ $course->passing_score }}%</p>
            @endif
          </div>
          <div class="col-md-6">
            <h6 class="text-uppercase text-secondary text-xs font-weight-bolder">Jadwal & Detail</h6>
            <p class="mb-2"><strong>Tanggal Mulai:</strong> {{ $course->start_date->format('d F Y') }}</p>
            <p class="mb-2"><strong>Tanggal Selesai:</strong> {{ $course->end_date->format('d F Y') }}</p>
            <p class="mb-2"><strong>Harga:</strong> <span class="text-success font-weight-bold">Rp {{ number_format($course->price, 0, ',', '.') }}</span></p>
            <p class="mb-2"><strong>Kuota:</strong> {{ $course->enrollments()->where('status', 'approved')->count() }}/{{ $course->max_participants }} Peserta</p>
          </div>
        </div>
