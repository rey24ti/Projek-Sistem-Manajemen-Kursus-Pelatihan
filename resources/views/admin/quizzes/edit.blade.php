      ...
      <div class="card-body">
        <form action="{{ route('courses.quizzes.update', [$course, $quiz]) }}" method="POST">
          ...
          <div class="mb-3">
            ...
            <textarea name="questions" class="form-control @error('questions') is-invalid @enderror" rows="10">{{ old('questions', $quiz->questions ? json_encode($quiz->questions, JSON_PRETTY_PRINT) : '') }}</textarea>
            @error('questions')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="datetime-local" name="start_date" class="form-control @error('start_date') is-invalid @enderror" value="{{ old('start_date', $quiz->start_date?->format('Y-m-d\\TH:i')) }}" required>
              @error('start_date')
              <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="datetime-local" name="end_date" class="form-control @error('end_date') is-invalid @enderror" value="{{ old('end_date', $quiz->end_date?->format('Y-m-d\\TH:i')) }}" required>
              @error('end_date')
              <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
