                ...
                <td class="align-middle text-center text-sm">
                  <div class="d-flex flex-column align-items-center">
                    ...
                    <div class="mt-2 d-flex gap-1">
                      ...
                      <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $enrollment->id }}">Reject</button>
                      @endif
...
<div class="modal fade" id="rejectModal{{ $enrollment->id }}" tabindex="-1" aria-labelledby="rejectModalLabel{{ $enrollment->id }}" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="rejectModalLabel{{ $enrollment->id }}">Tolak Pembayaran</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form action="{{ route('enrollments.reject-payment', $enrollment) }}" method="POST">
        @csrf
        <div class="modal-body">
          ...
          <p class="text-sm text-muted"><strong>Peserta:</strong> {{ $enrollment->user->name }}</p>
          <p class="text-sm text-muted"><strong>Kursus:</strong> {{ $enrollment->course->title }}</p>
          <p class="text-sm text-muted"><strong>Harga:</strong> Rp {{ number_format($enrollment->course->price, 0, ',', '.') }}</p>
        </div>
        <div class="modal-footer">
